...
func TestContextTokensBar_NilCurrentUsage(t *testing.T) {
	data := &schema.Input{}
...
func TestContextTokensBar_FallbackPctFromCurrentUsage(t *testing.T) {
	data := &schema.Input{}
	data.ContextWindow.ContextWindowSize = 200000
	data.ContextWindow.CurrentUsage = &schema.Usage{
		InputTokens: 5000, CacheReadInputTokens: 35000,
	}
	// no UsedPercentage — should compute from ContextFillTokens
	result := Get("context_tokens_bar").Render(data, config.Default(), theme.Get("default"))
